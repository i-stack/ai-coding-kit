...
- **kebab-case 旧名转发 shim（一个发布周期）**: v74 全量切换删除了 snake_case 入口，但仓库内历史提案仍含可复制旧命令（如 `cam-fields-preserve-format`、`register-gr011-013-rule-index`），仓库外调用方也无法被检索覆盖。现为 `ios-engineer/scripts/` 28 个脚本与全局 `detect_project_type.sh` / `skill_bundles.sh` 恢复旧名转发层：打印 `DEPRECATED` 后 `exec` 新名。测试断言旧入口存在、弃用提示与转发目标，防止再次静默删兼容层。
