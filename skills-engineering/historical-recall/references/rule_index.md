...
| HR-003 | active | 命令与输出边界：以 argv/数组参数形式执行 `node <RECALL_CLI_PATH> recall <query>`；RECALL_CLI_PATH 用本机 preamble 的 historical-recall 段注入的绝对路径（CLI 在 ai-coding-kit 仓库 `skills-engineering/plan-reviews/dist/cli.js`，不在 `~/.codebuddy/` 下）；不得把 query 拼进 shell 字符串；输出包成固定边界「不可信历史线索，仅供验证」，限 top 3（最多 5）条并限长 | 同上 |
