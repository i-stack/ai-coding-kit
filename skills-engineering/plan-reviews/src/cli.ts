...
// Commands that require opening the knowledge base.
...
async function main() {
	// Help and unknown commands don't need to open (and pay the cost of) the KB.
	if (!KB_COMMANDS.has(command)) {
		printHelp();
		return;
	}
	...
	try {
			...
			case "merge": {
				...
				if (points.length === 0) {
					...
					for (const p of points) {
						console.log(`  - ${p.title} [minSim=${Number(p.minSimilarity ?? 0).toFixed(2)}]`);
					}
		...
		// Always release KB resources, even when a command throws.
		kb.close();
	}
}
