...
- [CMR-003] **reviewer 只读**：每个 reviewer 必须以只读模式运行——codex 用 `-s read-only`，gemini 用 `--approval-mode plan`，claude 用 `--permission-mode plan`。reviewer 不写代码，只输出 `VERDICT: APPROVED` 或 `VERDICT: REVISE` + 具体修改建议。
