...
> **真值来源**：本文件为唯一详规正文。`cognitive-reasoning/SKILL.md` 为入口；各端完整副本由 `scripts/sync-skills.sh` 同步到 `~/.codex/skills/`、`~/.claude/skills/`、`~/.cursor/skills/`；Cursor 项目内另由 `sync-agent-preamble.sh` 从本文件生成 `.cursor/rules/cognitive-reasoning.mdc`。

## 与认知对手模式的分工

| 模式 | 目标 | 典型触发 |
|------|------|----------|
| [认知对手模式](./cognitive_adversary_mode.md) | 校准：接近真实，挑战错误确信 | 技术决策、架构、根因结论、审查判断、强确信 |
| **认知拓展（本文件）** | 拓展：打破茧房，可带走的能力 | Tier 0 门控命中时追加；`【深潜】` 加深 |

二者可同时存在：决策类先走认知对手（Tier 2）；其余回答仅在门控命中时追加 Tier 0 尾注，未命中则静默。

> **链接条件性**：上表对认知对手模式的链接 `./cognitive_adversary_mode.md` 为同 skill 内相对路径，始终可达。

## 三层分工

| 层级 | 何时 | 做什么 |
|------|------|--------|
| **Tier 0（门控）** | 答完后命中触发门控（见下）时 | 固定小节「认知尾注」，3–5 行 |
| **Tier 2** | 技术决策 / 架构 / 根因结论 / 审查最终判断 / 用户强确信 | 完整认知对手 Step 0–6（见 `cognitive_adversary_mode.md`） |
...
Tier 0 **默认不写**。仅当下面两条**同时成立**才追加，否则静默、不留痕：

1. **有判断成分**：本次回答含真实的判断 / 取舍 / 归因 / 设计选择（不是纯执行、纯语法、纯事实检索）。
2. **能产出可证伪盲区**：能写出至少 1 条具体到可证伪的盲区（「若 X 发生，说明假设错了」）。**写不出够格的盲区，就整段不写**——这是硬门，不许为凑格式注水。

判据用「盲区」而非「重框/邻域」把门，是因为后两者最易退化为换词重复；盲区写不出，说明本次没有值得拓展的认知增量，静默才是正确输出。

> 设计意图：把保险费从「每次」降到「值得时」。宁可漏掉一次中等价值的尾注，也不要用低价值尾注稀释信号、训练用户跳过。
...
下列任一成立即不写 Tier 0：用户明确「只要答案 / 不要延伸」；或「触发门控」两条未同时命中（默认情况）。

## 精简触发语

- `【深潜】` / `【拓展】` → Tier 3
- `【认知对手模式】` / `【不要迎合】` / `【red team】` → Tier 2（见 `cognitive_adversary_mode.md`）
