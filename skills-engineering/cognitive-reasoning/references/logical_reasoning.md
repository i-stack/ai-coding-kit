...
<!-- sha256: ee7be1a2614fd3b77c8cf569b04ae72211dc5bda67ae1cf9dd23043ed9515beb -->

# 逻辑性（Logical Reasoning）

## 适用场景

本文件是 `cognitive-reasoning` skill **[GR-010]** 的细则真值。所有**含判断成分**的回复均须满足（纯机械执行可跳过，见 SKILL.md「何时加载」），与认知对手模式（挑战用户逻辑）作用对象不同：本文件约束 **AI 自身** 的论证质量。
...
**填充示例**（根因归因场景，对照模板看「不写模板词」长什么样）：

```text
逻辑链
事实/证据：崩溃日志栈顶为 -[NSArray objectAtIndex:]，越界发生在列表刷新回调；该回调在后台队列触发，但数据源数组在主线程被同时 mutate（代码 L142 / L207）。
推断：因为读写跨线程且无同步，所以刷新读到中间态导致 count 与实际不符越界——是数据竞争，不是索引计算错误。
结论强度：明确。栈顶 + 双线程同时访问同一数组已构成充分证据。
可证伪/缺口：若用 TSan 跑不出该数组的 data race，或越界在单线程复现，则本判断被推翻。
```

**与 `验证锚点` 共存**：高风险任务常同时触发 `验证锚点`（GR-011）。二者字段重叠（「结论强度」=「置信度」、「可证伪/缺口」≈「怎么核」、「事实/证据」≈「依据来源」），**不要堆两个框**——合并为单一审计块、字段去重；有四段式时把推断并入「为什么」、证伪并入「验证」。完整合并规则见 engineering-discipline GR-004「多块合并」。
