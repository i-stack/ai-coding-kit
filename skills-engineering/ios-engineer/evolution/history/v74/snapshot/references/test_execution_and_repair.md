...
| **2. 自适应脚本** | `scripts/run-ios-tests.sh`（自动发现 workspace/scheme/simulator） | CI 流水线、本地手动回归、MCP 不可用时的回退 |
...
脚本位于 `skills-engineering/ios-engineer/scripts/run-ios-tests.sh`，会自动发现项目配置：
