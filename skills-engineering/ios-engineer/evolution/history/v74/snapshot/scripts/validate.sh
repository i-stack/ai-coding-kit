...
if $QUICK; then
  ...
  run_step "3/4" "Validate ref existence" bash scripts/validate-scenario-specs.sh <<<'skip'
  ...
  run_step "4/4" "Validate rule IDs" bash scripts/validate-rule-ids.sh
fi

if $SCENARIOS; then
  echo "=== Scenario Validation ==="
  run_step "S1" "Validate scenario specs" bash scripts/validate-scenario-specs.sh
...
if $LEDGER; then
  ...
  run_step "LD1" "Validate usage ledger" bash scripts/validate-usage-ledger.sh
fi

if $IDS; then
  echo "=== Rule ID Validation ==="
  run_step "ID1" "Validate rule IDs" bash scripts/validate-rule-ids.sh
fi
...
if $ALL; then
  ...
  SKIP_SNAPSHOT_CONSISTENCY="$SKIP_SNAPSHOT" \
    ...
    bash scripts/validate-skill-evolution.sh
  exit $?
fi
