...
# scripts/extract-usage-audit.sh.
#
# Usage: bash scripts/sync-transcript-to-ledger.sh <transcript-jsonl> [session-id]
#
...
if EXTRACT_OUT="$(bash scripts/extract-usage-audit.sh "$TMPTEXT" 2>&1)"; then
  echo "$TOTAL_LINES" > "$OFFSET_FILE"
  {
