...
#
# Usage:
#   bash scripts/install-codex-ledger-sync.sh install
#   bash scripts/install-codex-ledger-sync.sh uninstall
#   bash scripts/install-codex-ledger-sync.sh status

set -euo pipefail

SKILL_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SWEEP_SCRIPT="$SKILL_DIR/scripts/sync-codex-sessions.sh"
