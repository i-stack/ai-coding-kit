...
# 用法：bash scripts/test-gc-invariant.sh
# 失败退出非零。
...
cp "$SCRIPT_DIR/gc-evolution-history.sh" "$SCRIPT_ROOT/scripts/"
cp "$SCRIPT_DIR/rollback-skill-evolution.sh" "$SCRIPT_ROOT/scripts/"
...
if KEEP_RECENT=10 bash "$SCRIPT_ROOT/scripts/gc-evolution-history.sh" >/dev/null 2>&1; then
...
if bash "$SCRIPT_ROOT/scripts/rollback-skill-evolution.sh" v999 >/dev/null 2>&1; then
...
if bash "$SCRIPT_ROOT/scripts/rollback-skill-evolution.sh" "../../etc/passwd" >/dev/null 2>&1; then
