...
- 分层名称服从项目既有架构；不要因为规则里出现 `MVVM` 就强制改造。真正必须满足的是 UI、状态编排、业务规则、数据访问和基础设施职责可区分、可测试、可替换。

### 依赖方向
- UI 层依赖业务抽象，不反向依赖具体实现。
- 高层模块不得导入低层实现细节。
- 通过构造器注入依赖；容器注入只用于装配，不用于隐藏依赖。
- 跨模块通信优先通过协议、路由能力、UseCase / Repository 抽象或项目既有中介机制完成；不把“中间件”作为强制形态，也不允许绕过边界直接访问对方内部实现。
...
- 不允许“跨模块直接访问内部实现”式偷渡。
- 模块拆分必须先定义稳定输入输出、模块负责人和依赖方向；只为缩短文件长度而拆包，不算有效模块化。
- 基础层只能放跨业务稳定能力；业务基础层只能放多个业务共同依赖的业务抽象；业务组件层不得反向污染基础层。
...
- 新增模块边界或公共 API 时，必须说明最小测试切面：单元测试覆盖业务规则，集成测试覆盖跨模块调用链，UI / 快照验证覆盖可见状态变化。
...
- [ ] 该设计是否便于测试、扩展和排障？
- [ ] 公共 API、模块通信方式、测试切面和 CI 门禁是否随边界变化同步说明？
