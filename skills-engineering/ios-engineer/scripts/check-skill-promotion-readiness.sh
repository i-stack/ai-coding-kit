...
if [ $# -lt 1 ]; then
  echo "Usage: bash scripts/check-skill-promotion-readiness.sh <proposal-file>"
  exit 1
fi
...
if [ "$proposal_status" = "ready_to_promote" ] && [ "$promotion_readiness" = "ready_to_promote" ] && [ "$approval_status" = "missing" ]; then
  echo "hint=Proposal is ready. To authorize promotion, run: bash scripts/approve-skill-promotion.sh ${proposal_file} \"approved-by-user\""
fi
