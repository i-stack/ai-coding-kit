...
if [ $# -lt 1 ]; then
  echo "Usage: bash scripts/validate-skill-proposal.sh <proposal-file> [scenario-slug ...]"
  echo "Example: bash scripts/validate-skill-proposal.sh evolution/proposals/20260403-fix.md layout parameter-pass-through"
  exit 1
fi
...
set +e
SKIP_SNAPSHOT_CONSISTENCY=1 bash scripts/validate-skill-evolution.sh >"$tmp_output" 2>&1
exit_code=$?
set -e
...
bash scripts/update-skill-proposal-status.sh "$proposal_file" "$status" >/dev/null
cat "$record_file"
