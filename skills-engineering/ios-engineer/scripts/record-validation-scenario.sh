...
if [ $# -lt 6 ]; then
  echo "Usage: bash scripts/record-validation-scenario.sh <proposal-file> <scenario> <result> <hits> <deviations> <improvements>"
  echo 'Example: bash scripts/record-validation-scenario.sh evolution/proposals/20260403-fix.md layout pass "命中根因四段式;先看复用链路" "无" "无"'
  exit 1
fi
...
bash scripts/update-skill-proposal-status.sh "$proposal_file" "$next_status" >/dev/null
cat "$record_file"
