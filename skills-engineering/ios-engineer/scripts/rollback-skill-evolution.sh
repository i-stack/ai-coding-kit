...
if [ $# -lt 1 ]; then
  echo "Usage: bash scripts/rollback-skill-evolution.sh <version>"
  exit 1
fi
...
# 5. 完整 validate
if ! bash scripts/validate-skill-evolution.sh; then
...
# 8. 刷新完整性基线：rollback 把快照恢复进工作副本，真实改动了 SKILL.md /
#    references / agents 等契约内容，必须显式登记（不带 --check-only 即刷新），
#    否则随后的 commit 会在 CI 的 --check-only 上漂移失败。
