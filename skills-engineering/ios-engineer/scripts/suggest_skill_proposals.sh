#!/usr/bin/env bash
# =============================================================================
# suggest_skill_proposals.sh — Skill 自我改进闭环 (观测 → 建议 → 人工审批)
#
# 对齐 Hermes Agent 的「学习循环」思路，但落地为本仓库既有的「受控演进」闸门：
#   1. 读取 summarize_usage_ledger.sh --json 产出的 proposal_signals
...
#   bash scripts/suggest_skill_proposals.sh --dry-run  # 只打印会生成什么，不落盘
#   bash scripts/suggest_skill_proposals.sh --json     # 机器可读输出
# =============================================================================
set -euo pipefail
...
SUMMARIZE="scripts/summarize_usage_ledger.sh"
...
- **Active Version At Creation**: (待填充 — 运行 create_skill_proposal.sh 风格元数据)
- **Status**: draft
- **Auto-generated**: true
- **Signal key**: {key}

## 问题信号
- 来源：usage ledger 汇总信号（summarize_usage_ledger.sh）。
- {note}
...
## 验证
- 结构校验：`bash ios-engineer/scripts/validate_skill_evolution.sh`
- 场景回放：必要时 `bash ios-engineer/scripts/validate_skill_proposal.sh evolution/proposals/{pid}.md`
...
print("")
print(f"共生成 {len(generated)} 份草稿，跳过 {len(skipped)} 份。运行 create_skill_proposal.sh / approve 流程前请先 review。")
PY
