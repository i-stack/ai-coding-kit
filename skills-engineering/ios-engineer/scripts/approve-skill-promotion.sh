...
if [ $# -lt 2 ]; then
  echo "Usage: bash scripts/approve-skill-promotion.sh <proposal-file> <approved-by>"
  echo 'Example: bash scripts/approve-skill-promotion.sh evolution/proposals/20260403-fix.md "approved-by-user"'
  exit 1
fi
...
bash scripts/update-skill-proposal-status.sh "$proposal_file" approved >/dev/null
cat "$approval_file"
