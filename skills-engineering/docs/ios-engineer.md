...
### 1. 认知对手模式
当涉及技术决策、架构取舍、根因归因、审查最终判断或用户强烈确信时，自动启动认知校准流程（Step 0–6），优先接近真实而非维持对话和谐。该模式作为 platform-agnostic 能力，其真值 owner 为 `cognitive-reasoning` skill（合并自 cognitive-calibration 等四技能），`ios-engineer` 经 `depends_on` 引用（详见该 skill）。
